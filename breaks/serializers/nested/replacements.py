...
class ReplacementMemberShortSerializer(ExtendedModelSerializer):
    id = serializers.CharField(source='member.employee.user.id')
    ...
    def get_description(self, instance):
        ...
        break_start = datetime.datetime.combine(now.date(),
                                                instance.break_start).astimezone()
        break_end = datetime.datetime.combine(now.date(),
                                              instance.break_end).astimezone()

        if break_start > now:
            delta = break_start - now
            return f'Обед начинается через {convert_timedelta_to_str_time(delta)}'
        elif break_end > now:
            delta = break_end - now
