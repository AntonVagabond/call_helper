...
class ReplacementRetrieveSerializer(InfoModelSerializer):
    ...
    def get_stats(self, instance):
        result = self.Meta.model.objects.filter(pk=instance.pk).aggregate(
            ...
            breaks_count=Count(expression='breaks', distinct=True),
            members_online=Count(
                expression='members',
                filter=Q(members_info__status_id=REPLACEMENT_MEMBER_ONLINE),
                distinct=True
            ),
            members_offline=Count(
                expression='members',
                filter=Q(members_info__status_id=REPLACEMENT_MEMBER_OFFLINE),
                distinct=True
            ),
            members_busy=Count(
                expression='members',
                filter=Q(members_info__status_id=REPLACEMENT_MEMBER_BUSY),
                distinct=True
            ),
            members_break=Count(
                expression='members',
                filter=Q(members_info__status_id=REPLACEMENT_MEMBER_BREAK),
                distinct=True
            ),
        )
        return result
