...
class BreakMeUpdateSerializer(InfoModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if 'break_start' in attrs and 'break_end' in attrs:
            ...
            break_start = datetime.datetime.combine(datetime.date.today(),
                                                    attrs['break_start'])
            break_end = datetime.datetime.combine(datetime.date.today(),
                                                  attrs['break_end'])
            ...
            if not instance_id:
                if replacement.breaks.filter(member=member).exists():
                    raise ParseError('Вы уже зарезервировали обеденный перерыв.')
    ...
    def validate_status(self, value):
        ...
        if self.instance.member.status_id == REPLACEMENT_MEMBER_OFFLINE:
            raise ParseError('Невозможно начать обед, пока Ваш статус Офлайн.')
        ...
        if value == 'break_start':
            ...
            if now + datetime.timedelta(minutes=5) < break_start:
                raise ParseError('Время обеденного перерыва ещё не началось.')

            if self.instance.member.time_break_start:
                raise ParseError('Обеденный перерыв уже начался.')

        else:
            if not self.instance.member.time_break_start:
                raise ParseError('Обеденный перерыв ещё не начался.')

            if self.instance.member.time_break_end:
                raise ParseError('Обеденный перерыв уже закончился.')
...
class BreakScheduleSerializer(serializers.Serializer):
    ...
    def get_post_blank(self, instance):
        span = self._get_span_count(instance.break_end,
                                    instance.replacement.break_end)
        return self._convert_to_cell(span=span)

    @staticmethod
    def _convert_to_cell(value='', color='#fff', span=None):
    ...
    @staticmethod
    def _get_span_count(start_board, start_instance):
