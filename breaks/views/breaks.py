...
@extend_schema_view(
    get=extend_schema(
        summary='Детали обеда',
        tags=['Обеды: Обеды пользователя'],
    ),
    post=extend_schema(
        summary='Резерв обеда',
        tags=['Обеды: Обеды пользователя'],
    ),
    patch=extend_schema(
        summary='Изменение резерва обеда',
        tags=['Обеды: Обеды пользователя'],
    ),
)
...
class BreakScheduleView(ListViewSet):
    ...
    def get_queryset(self):
        replacement_id = self.request.parser_context['kwargs'].get('pkx')
        return Break.objects.prefetch_related(
            'member',
            'member_member',
            'member_member_employee',
            'member_member_employee_user',
