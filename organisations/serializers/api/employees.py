...
class EmployeeUpdateSerializer(ExtendedModelSerializer):
    ...
    def validate(self, attrs):
        if self.instance.is_director:
            raise ParseError('Руководитель организации недоступен для изменений.')
        return attrs

    def validate_position(self, value):
        if value.code == OPERATOR_POSITION:
            if self.instance.is_manager:
                employee_groups = self.instance.groups_managers.values_list('name',
                                                                            flat=True)
                if employee_groups:
...
class EmployeeDeleteSerializer(serializers.Serializer):
    def validate(self, attrs):
        if self.instance.is_director:
            raise ParseError('Невозможно удалить руководителя из организации.')

        groups_as_member = self.instance.groups_members.values_list('name',
                                                                    flat=True)
        groups_as_manager = self.instance.groups_managers.values_list('name',
                                                                      flat=True)
