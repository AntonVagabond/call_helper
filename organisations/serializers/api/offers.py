...
class OfferOrgToUserCreateSerializer(ExtendedModelSerializer):
    users = serializers.PrimaryKeyRelatedField(
        queryset=User.objects.exclude(is_corporate_account=True),
        many=True,
        write_only=True,
    )
    ...
    def validate(self, attrs):
        ...
        if offers_exist:
            user_error = '\n'.join(
                [f'{offer.user.full_name} ({offer.user.email})'
                 for offer in offers_exist]
            )
            raise ParseError(
        ...
        if users_in_orgs:
            user_error = '\n'.join([employee.user.full_name
                                    for employee in users_in_orgs])
            raise ParseError(
...
class OfferUserToOrgCreateSerializer(ExtendedModelSerializer):
    ...
    def validate(self, attrs):
        ...
        # check offer create already
        offer_exists = self.Meta.model.objects.filter(
            organisation=organisation,
            user=user,
        ).exists()
        if offer_exists:
            raise ParseError(f'Заявка в эту организацию была отправлена раннее.')

        # check user in org already
        already_in_org = organisation.employees_info.filter(user=user).exists()
        if already_in_org:
            raise ParseError(f'Вы уже являетесь сотрудником организации.')
        return attrs
