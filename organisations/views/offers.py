...
@extend_schema_view(
    ...
    create=extend_schema(summary='Создать офферы пользователям', tags=['Организации: Офферы']),
    partial_update=extend_schema(summary='Частично изменить оффер пользователя', tags=['Организации: Офферы']),
)
